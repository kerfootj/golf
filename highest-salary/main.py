#
# https://codegolf.stackexchange.com/questions/171168/without-using-numbers-get-the-highest-salary-you-can-but-dont-exaggerate
#
# Without using numbers, get the highest salary you can. But don't exaggerate!
#

# attempt 2 - 17 bytes - $119200
f"${ord('𝆠')}" # probably needs a print so it can be run in as a file and not from the python cli

# attempt 2.1 - 24 bytes - $92223
print(f"${ord('𖠿')}")

# attempt 2.3 - 22 bytes - $98442
 print('$',ord('𘂊'))